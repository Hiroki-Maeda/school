...
Seq *srchS(Seq *sq,char *startcdn){
    ...
    Seq *p;
    Seq *temp;
        for(p=bp;(((p->next)->next)->next)!=NULL;p=p->next){
            
            if((p->nc)==startcdn[0]&&((p->next)->nc)==startcdn[1]&&(((p->next)->next)->nc)==startcdn[2]&&((((p->next)->next)->next)->nc)==startcdn[3]){
                return p;
            }
        }
    return NULL;
